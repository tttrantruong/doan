...
@if(!empty(session()->get("messThemNhanVien")))
<script>
    alert("Thêm Nhân Viên Thành Công !!!");
</script>
@php
session()->forget('messThemNhanVien');
@endphp
@endif

@if(!empty(session()->get("messXoaNhanVien")))
<script>
    alert("Xóa Nhân Viên Thành Công !!!");
</script>
@php
session()->forget('messXoaNhanVien');
@endphp
@endif

@if(!empty(session()->get("messCapNhatNhanVien")))
<script>
    alert("Cập Nhât Nhân Viên Thành Công !!!");
</script>
@php
session()->forget('messCapNhatNhanVien');
@endphp
@endif
                        ...
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                <span class="tools pull-right">
                                    <a href="{{route('admin.themNhanVien')}}" class="btn btn-info" style="color: white;">Thêm Nhân Viên</a>
                                </span>
                                <br> <br>
                            </header>
                                    ...
                                    <table class="display table table-bordered table-striped" id="dynamic-table">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="gradeX">
                                                ...
                                                <td class="center hidden-phone"><img src="{{url($nhanVien->ImgNhanVien)}}" style="width: 60px; height: 60px;" alt=""></td>
                                                <td>{{$nhanVien->SDTNhanVien}}</td>
                                                <td>{{$nhanVien->MailNhanVien}}</td>
                                                <td>{{$nhanVien->Role}}</td>
                                                <td>{{$nhanVien->Active}}</td>
                                                <td class="center hidden-phone">
                                                    @if($nhanVien->Active=="Hoạt Động")
                                                    <a title="Cập Nhật" href="{{url('/admin/tai-khoan/khoa')}}/{{$nhanVien->IDNhanVien}}" class="btn btn-danger btn-xs">Khóa Tài Khoản</a>
                                                    @else
                                                    <a title="Xóa" href="{{url('/admin/tai-khoan/kich-hoat')}}/{{$nhanVien->IDNhanVien}}" class="btn btn-primary btn-xs">Kích Hoạt Tài Khoản</a>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endforeach
