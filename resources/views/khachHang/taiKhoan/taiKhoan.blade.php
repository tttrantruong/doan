...
@if(!empty(session()->get("messDangky")))
<script>
    alert("Bạn Đã Đăng Ký Tài Khoản Thành Công !!!");
</script>
@php
session()->forget('messDangky');
@endphp
@endif
<div id="main-content-wp" class="checkout-page">
	...
	<div id="wrapper" class="wp-inner clearfix">
		...
		<form method="post" action="{{route('dangNhapP')}}" name="form-checkout">
			{{ csrf_field() }}
		...
		<div class="section" id="customer-info-wp">
			...
			<div class="section-detail">
				...
				<form method="POST" action="{{route('dangKyP')}}" name="form-checkout">
					{{ csrf_field() }}
					<div class="form-row clearfix">
						<div class="form-col fl-left">
							<label for="fullname">Tên đăng nhập</label>
					...
					<div class="form-row clearfix">
						<div class="form-col fl-left">
							...
							<input type="text" name="MailKhachHang" id="address">
						</div>
						<div class="form-col fl-right">
							<label for="phone">Số điện thoại</label>
							<input type="tel" name="SDTKhachHang" id="phone">
						</div>
					</div>
					<div class="form-row">
						<label for="phone">Họ và tên</label>
						<input style="display: inline-block;padding-bottom: 10px; width: 100%;border: 1px solid #cccccc;" name="TenKhachHang" id="">
						<div class="form-col" style="width: 100%; padding-right: 0px;">
							<label for="notes">Địa chỉ</label>
							<textarea style="width: 100%;" rows="5"  name="DiaChiKhachHang"></textarea>
						</div>
					</div>
					<input type="submit" name="btn_submit_crate" id="btn-submit" value="Đăng Ký" style="height: 40px;font-size: 16px;
                                                                                                width: 150px;
                                                                                                color: green;
                                                                                                border-color: white;
                                                                                                color: white;
